...
class main:
    ...
    def __init__(self):
        ...
        self.avm_size = (260, 480)
        self.avm_surface = pygame.surface.Surface(self.avm_size, 0, 24).convert()
        ...
        if self.joystick.get_name() == "Logitech PRO Racing Wheel":
            print("Logitech PRO Racing Wheel Connected")

            self.isPROWheel = True
        else:
            print("Logitech G29 Connected")
            self.isPROWheel = False
        pass
    ...
    def get_joy_and_send(self): # 조이스틱 핸들 페달 값 받아서 차로 전송
        while 1:
            try:
                ...
                if self.isPROWheel:
                    # CAUTION! Accel And brake is reversed
                    tmp = [tmp[0],tmp[3],tmp[1],tmp[2]]
                ...
                if AUTO_CENTER and not self.isPROWheel: self.g29_auto_center_daemon.loop()
        ...
        while(1):
            try:
                ...
                if int(self.sN.number_of_camera) == 1 and self.sN.camera_init:
                    ...
                    try:
                        img = hud.hud(self.sN.sub_data["sas_angle"], round(self.sN.sub_data["vEgo_in_ms"], 1), "D", None, img, float(config["hud"]["camera_offset"]))
                    ...
                    avm_img = self.sN.get_camera_img("avm")
                    avm_img = cv2.cvtColor(avm_img, cv2.COLOR_BGR2RGB)
                    pygame.surfarray.blit_array(self.avm_surface, avm_img.swapaxes(0, 1))
    ...
    def graph_draw(self):
        ...
        screen.blit(self.draw_plots(plot_arr), (30, 700))
    ...
    def ui_loop(self):
        ...
        self.bg_init()
        if self.sN.camera_init : 
            screen.blit(self.camera_surface, (30, 30))
            screen.blit(self.avm_surface, (1080+130, 30))
    ...
    def init_plots(self, arr, name_to_arr_idx, plot_xlims, plot_ylims, plot_names, plot_colors, plot_styles):
        ...
        if IS_DARWIN:
            ...
            dpi = 50
            fig = plt.figure(figsize=(350 / dpi, 250 / dpi), dpi=dpi)
...
class feed_provider:
    ...
    def camera_connection_worker(self):
        ...
        if VIDEO_COMM_MODE == 'udp':
            ...
            'udpsrc port=5118 caps = "application/x-rtp, encoding-name=H265, payload=96" buffer-size=108000'
            ' ! rtpjitterbuffer latency=0'
            ' ! rtph265depay'
            ' ! avdec_h265'
            ' ! decodebin'
        ...
        # avm value recv.
        self.avm_cap = cv2.VideoCapture(
            'udpsrc port=5000 caps = "application/x-rtp, encoding-name=H265, payload=96" buffer-size=108000'
            ' ! rtpjitterbuffer latency=0'
            ' ! rtph265depay'
            ' ! avdec_h265'
            ' ! decodebin'
        ...
        if self.avm_cap.read()[0]:
            self.sub_camera.update({"avm" : self.avm_cap})
        else:
            fmtc_remote.error_log.put(["AVM Open Error", ""])
